...
@section('page-subtitle', 'Monitor order preparation and serving status')
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            ...
            <p class="text-gray-500 mt-1">
                New orders start as pending. Update status here while KDS is not yet available.
            </p>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="p-5 border-b border-gray-100">
            ...
            <p class="text-sm text-gray-500">
                Use the status selector to test the order flow before the Kitchen Display System is added.
            </p>
        </div>
            ...
            <table class="w-full min-w-[950px] text-sm">
                <thead class="bg-gray-50 border-b border-gray-100">
                    <tr class="text-left text-gray-500">
                        ...
                        <th class="px-5 py-4 font-semibold text-right">Controls</th>
                    </tr>
                </thead>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($orders as $order)
                        ...
                        <tr class="align-top hover:bg-gray-50 transition">
                            ...
                            <td class="px-5 py-4">
                                <form method="POST" action="{{ route('service.active-orders.update-status', $order) }}"
                                      class="flex justify-end gap-2">
                                    @csrf
                                    @method('PATCH')

                                    <select name="status"
                                        class="rounded-lg border-gray-200 text-xs font-semibold focus:border-orange-300 focus:ring-orange-200">
                                        <option value="pending" {{ $currentStatus === 'pending' ? 'selected' : '' }}>
                                            Pending
                                        </option>
                                        <option value="preparing" {{ $currentStatus === 'preparing' ? 'selected' : '' }}>
                                            Preparing
                                        </option>
                                        <option value="ready" {{ $currentStatus === 'ready' ? 'selected' : '' }}>
                                            Ready
                                        </option>
                                        <option value="served" {{ $currentStatus === 'served' ? 'selected' : '' }}>
                                            Served
                                        </option>
                                    </select>
